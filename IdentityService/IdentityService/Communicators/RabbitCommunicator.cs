...
public class RabbitCommunicator: IRabbitCommunicator, IDisposable
{
    ...
    public RabbitCommunicator(IConfiguration configuration, ILogger logger, IRabbitMessageHandler messageHandler)
    {
    ...
    public void SendMessageToQueue(string queueName, RabbitMessage message)
    {
    ...
    public void SendMessageToExchange(string exchangeName, RabbitMessage message)
    {
    ...
    public RabbitMessage ReceiveMessageFromQueue(string queueName)
    {
        _channel.QueueDeclare(
            queue: queueName,
            durable: false,
        ...
        consumer.Received += (model, ea) =>
        {
            ...
            message = JsonConvert.DeserializeObject<RabbitMessage>(System.Text.Encoding.UTF8.GetString(body));
        };
    ...
    public RabbitMessage ReceiveMessageFromExchange(string exchangeName)
    {
        //Fanout exchange 
        _channel.ExchangeDeclare(exchangeName, type: ExchangeType.Fanout);
        ...
        consumer.Received += (model, ea) =>
        {
            ...
            _logger.Information("{}", messageJson);
